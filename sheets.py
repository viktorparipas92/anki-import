...
def fill_columns(
    ...
    """Fill every row with an empty `trigger_columns` cell with what `get_values` returns.
    ...
    Cells that already have a value are never read from or written to. A trigger
    column the sheet does not have is named and then left out.
    """
    ...
    if GENERATED_SHEET_COLUMN in header:
        raise ValueError(
            f'Sheet "{sheet_name}" is generated by formulas from another sheet, so '
            f'writing to it would overwrite them. Fill in that sheet instead.'
        )

    headword_column = _get_column(header, headword_columns)
    triggers = _get_trigger_columns(header, trigger_columns, sheet_name)
    joined_triggers = ', '.join(f'"{header[trigger]}"' for trigger in triggers)
    print(f'Headword column: "{header[headword_column]}". Triggers: {joined_triggers}')

    row_numbers = _get_empty_rows(rows, headword_column, triggers, limit)
    if not row_numbers:
        print(f'Nothing to fill: every word already has {joined_triggers}.')
        return [], []

    print(f'{len(row_numbers)} rows missing {joined_triggers}')
    fills = []
    missing = []
    pending = []
...
def _get_trigger_columns(
    header: list[str], trigger_columns: tuple[str, ...], sheet_name: str
) -> list[int]:
    """Resolve the trigger columns the sheet has, and name the ones it lacks."""
    present = [header.index(name) for name in trigger_columns if name in header]
    absent = [name for name in trigger_columns if name not in header]
    if absent:
        joined = ', '.join(f'"{name}"' for name in absent)
        print(f'Sheet "{sheet_name}" has no {joined} column, so it is left unfilled.')

    if not present:
        raise ValueError(
            f'No column found. Expected one of {list(trigger_columns)} in {header}.'
        )
...
def _get_empty_rows(
    ...
    row_numbers = [
        ...
        and any(not row[column].strip() for column in target_columns)
    ]
