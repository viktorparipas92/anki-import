...
Fills empty cells only. Swedish from Svensk ordbok and Wiktionary, the rest from
WordReference.

```bash
python -m scripts.fill_translations FRA Collection --dry-run
python -m scripts.fill_translations FRA Collection
python -m scripts.fill_translations Mixed FRA
python -m scripts.fill_translations SWE Input
```

Italian words live in `Nouns`, `Adjectives` and `Verbs`; its other sheets are
generated by formulas and are refused.

```bash
python -m scripts.fill_translations ITA --dry-run
python -m scripts.fill_translations ITA
python -m scripts.fill_translations ITA Adjectives
```
...
lacks. The A2-B1 book is a scan: `pip install -r requirements-ocr.txt`.

```bash
python -m scripts.import_french_glossary <B2-C1 PDF> --word-pages 12-15 --index-pages 187-209 --chapter 2 --tag B2-C1 --dry-run
python -m scripts.import_french_glossary <B2-C1 PDF> --word-pages 12-15 --index-pages 187-209 --chapter 2 --tag B2-C1
python -m scripts.import_french_glossary <A2-B1 PDF> --word-pages 6-11 --chapter 1 --dry-run
