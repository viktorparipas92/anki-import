...
A row is looked up when its `English` or `Pronunciation` is empty.
...
Italian words live in `Nouns`, `Adjectives` and `Verbs`. Leave the sheet out to
fill all three; its other sheets are generated by formulas and are refused.
